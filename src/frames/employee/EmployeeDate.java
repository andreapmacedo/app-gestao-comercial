...
public class EmployeeDate extends JPanel {

    private EmployeeDateItem cmpItens;
    private CMP_CMD_Confirm cmpCMD;
    public EmployerDataDAO itenDAO;
    private EmployeeData iten;


    DateFormat formatador = new SimpleDateFormat("dd-MM-yyyy");

    EstateDAO itenEstadoDAO = new EstateDAO();
    CityDAO itenCidadeDAO = new CityDAO();

    public EmployeeDate() throws SQLException, ParseException {
    ...
    private void initComps() throws SQLException, ParseException {
        ...
        cmpItens = new EmployeeDateItem(this);
    ...
    private void reloadForm() {
        ...
        try {
            ...
            Logger.getLogger(EmployeeDate.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public void loadItens() throws SQLException {

        List<EmployeeData> list; //o ideal é não fazer com lista pois é desnecessário. mas fica o exemplo

        list = new ArrayList(itenDAO.Select_By_idFK(EmployeeInfo.idEmployer));

        if (list.isEmpty()) {

        } else {

            for (EmployeeData iten : list) {
    ...
    private void event() {
        ...
        cmpCMD.cmdConfirm.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                iten = new EmployeeData();

                try {

                    List<EmployeeData> list = itenDAO.Select_By_idFK(EmployeeInfo.idEmployer);
                    ...
                    if (list.isEmpty()) {
                        ...
                        itenDAO.select_ID(EmployeeInfo.idEmployer).getIdPk(); //setar o id pk
                        iten.setIdPk(itenDAO.select_ID(EmployeeInfo.idEmployer).getIdPk());
                    }
